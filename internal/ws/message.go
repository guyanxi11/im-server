// Package ws（message.go）定义 WebSocket 消息协议
// 作者: wym
//
// 客户端 -> 服务端（上行）：
...
//
// 服务端 -> 客户端（下行）：
//   {"type":"chat","from":2,"from_name":"wym","to":3,"content":"在吗","ts":...}
//   {"type":"group_chat","from":2,"from_name":"wym","group_id":1,"content":"大家好","ts":...}
...
const (
	...
	MsgTypeGroupChat = "group_chat" // 群聊文本消息
	MsgTypePing      = "ping"       // 应用层心跳请求
	MsgTypePong      = "pong"       // 应用层心跳响应
	MsgTypeAck       = "ack"        // 投递回执
...
type InboundMessage struct {
	...
	GroupID uint   `json:"group_id"` // 群聊目标群 ID
...
type OutboundMessage struct {
	...
	TS       int64  `json:"ts,omitempty"` // Unix 秒级时间戳
}

// encodeOutbound 把下行消息序列化为 JSON 字节，供塞入 Client.send
